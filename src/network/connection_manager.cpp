...
namespace libp2p::network {
  ...
  ConnectionManager::getConnectionsToPeer(const peer::PeerId &p) const {
  ...
  void ConnectionManager::addConnectionToPeer(
  ...
  void ConnectionManager::closeConnectionsToPeer(const peer::PeerId &p) {
    auto it = connections_.find(p);
    if (it == connections_.end()) {
      return;
    }
