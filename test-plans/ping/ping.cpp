...
redisContext *connect_redis(const std::string &host,
                            int port,
                            int timeout_ms,
                            libp2p::log::Logger log) {
...
int parse_redis_port(std::string &redisAddr, libp2p::log::Logger log) {
  ...
  auto [ptr, ec] = std::from_chars(
      redisPortStr.data(), redisPortStr.data() + redisPortStr.size(), port);
  if (ec == std::errc{}) {
    return port;
  } else {
...
std::string parse_redis_host(const std::string &redisAddr,
                             libp2p::log::Logger log) {
  size_t colonPos = redisAddr.find(':');
  if (colonPos == std::string::npos) {
    log->warn(
        "No port separator found in redis address '{}', treating entire string "
        "as host",
        redisAddr);
    return redisAddr;
  }
...
int main() {
  ...
  redisContext *ctx =
      connect_redis(redisHost,
                    redisPort,
                    testTimeoutSeconds * 1000,
                    log);  // Redis connection needs timeout in ms
  ...
  if (*transport == "quic-v1") {
    ...
    host->listenProtocol(
        injector.create<std::shared_ptr<libp2p::protocol::Ping>>());
  } else {
  ...
  auto ping = std::make_shared<libp2p::protocol::Ping>(
      io_context, host, random, pingConfig);
  ...
  if (isDialer) {
    ...
    if (replyListenAddr) {
      ...
      bool ok = replyListenAddr->type == REDIS_REPLY_ARRAY
             && replyListenAddr->elements == 2;
      if (ok) {
        ...
        libp2p::coroSpawn(*io_context, [&]() -> libp2p::Coro<void> {
          ...
          if (not connect_res.has_value()) {
            ...
            auto ping_res = (co_await ping->ping(
                connection, std::chrono::seconds(testTimeoutSeconds)));
            if (not ping_res.has_value()) {
              ...
              double handShakePlusOneRTT_ms =
                  handShakePlusOneRTT_us.count() / 1000.0;
        ...
        log->error(
            "Failed to wait for listener to be ready - unexpected reply type: "
            "{} (elements: {})",
            replyListenAddr->type,
            replyListenAddr->type == REDIS_REPLY_ARRAY
                ? replyListenAddr->elements
                : 0);
        freeReplyObject(replyListenAddr);
        redisFree(ctx);
        return 1;
    ...
    std::string connectStr =
        std::string(sample_peer.connect.getStringAddress());
    log->info("Pushing connect string {}", connectStr);
    redisReply *replyListenAddr = (redisReply *)redisCommand(
        ctx, "RPUSH %s %s", redisKey.c_str(), connectStr.c_str());
    if (replyListenAddr) {
      if (replyListenAddr->type == REDIS_REPLY_ERROR) {
      ...
      if (ok) {
        ...
        log->error(
            "Failed to push address to redis - unexpected reply type: {}",
            replyListenAddr->type);
        freeReplyObject(replyListenAddr);
        redisFree(ctx);
        return 1;
