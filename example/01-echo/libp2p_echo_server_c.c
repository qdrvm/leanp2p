 ...
 * This example demonstrates how to create a basic echo server using the libp2p C API
...
void on_write_complete(libp2p_stream_t* stream, size_t bytes_written,
                       libp2p_error_t error, void* user_data);

// Read callback - called when data is read from the stream
void on_read_complete(libp2p_stream_t* stream, const uint8_t* data, size_t size,
                      libp2p_error_t error, void* user_data) {
    ...
    libp2p_error_t write_result = libp2p_stream_write(stream, data, size,
                                                      on_write_complete, user_data);
...
void on_write_complete(libp2p_stream_t* stream, size_t bytes_written,
    ...
    libp2p_error_t read_result = libp2p_stream_read(stream, state->read_buffer,
                                                    sizeof(state->read_buffer),
                                                    on_read_complete, user_data);
...
// Echo protocol stream handler - called when a new stream is opened with the echo protocol
...
int main(int argc, char* argv[]) {
    ...
    libp2p_error_t result = libp2p_host_register_protocol(host, "/echo/1.0.0",
                                                          echo_protocol_handler, NULL);
    if (result != LIBP2P_SUCCESS) {
    ...
    if (result != LIBP2P_SUCCESS) {
        printf("Failed to listen on %s: %s\n", listen_addr, libp2p_error_string(result));
        libp2p_host_destroy(host);
        libp2p_context_destroy(g_context);
        return 1;
    }
    ...
    if (result != LIBP2P_SUCCESS && result != LIBP2P_SUCCESS) {
